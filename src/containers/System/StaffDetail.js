...
import { getTourByStaffID } from '../../store/actions/tourPlaceAction';
import { sorting } from '../../ultils/sorting';
import { staffActivate } from '../../store/actions/userAction';
...
const StaffDetail = () => {
    ...
    const handleDeactivate = () => {
        if (tours_staff.length > 0 && tours_staff.filter(item => item.isActive === true).length > 0) {
            Swal.fire({
            ...
            <div className='relative text-body-1 text-[#363837] grid gap-6 mx-auto px-4 py-6 border-[3px] border-secondary-2 rounded-b-2xl rounded-tr-2xl md:grid-cols-2 xl:gap-40 xl:w-[900px]'>
                ...
                <div className='flex flex-col gap-6'>
                    <div className='flex gap-2 items-center'>
                        ...
                        <div className='font-normal'>{staff?.phone_no}</div>
                    </div>
                    <div className='flex gap-2 items-center'>
