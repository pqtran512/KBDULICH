...
const StaffList = () => {
    ...
    const currentStaff = sortData.slice(indexOfFirst, indexOfLast);
    const exportToExcel = () => {
        const data = [...staffs]
        ...
        <div className='w-full px-6 pt-20 pb-10 xl:pt-7 xl:pb-20 xl:pl-0 xl:pr-10 overflow-x-hidden'>
            <div className='pb-10 flex flex-col gap-y-5 md:flex-row md:items-center md:justify-between'>
                ...
                <div className='ml-auto xl:ml-0'>
                    <SearchBar placeholder='Nhập ..' newPlaceholder='Nhập tìm kiếm . . .' width='w-10 md:w-24 xl:w-28' change={true} newWidth='w-[380px] xl:w-[500px]' 
                        optionBar={true} person={true} email={true} path={location.pathname} staffs={staffs} setOutput={setSortData}/>
                </div>
            </div>
            <div className='pb-6 flex flex-col gap-2 md:flex-row md:justify-between md:items-end'>
                <div className='flex gap-10'>
                    ...
                    <div className='text-body-2 text-neutral-1-900 h-fit px-3 bg-background-7 rounded-xl xl:text-body-1'>Số nhân viên đang hoạt động: <span className='font-semibold'>{staffs?.filter(item => item.isActive === true).length}</span></div>
                </div>
                ...
                <thead>
                    <tr className="h-10 font-semibold tracking-wider bg-neutral-3-200">
                        ...
                        <td className="xl:min-w-[66px]">
                            <div className='flex items-center gap-1'>
                                ...
                                <CgArrowsExchangeAltV size={24} className='text-neutral-1-200 rounded-md hover:text-neutral-1-300 hover:bg-neutral-3-300 cursor-pointer'
                                    onClick={() => sorting("staff_ID", staffs, setSortData, order, setOrder)}/> 
                            </div>
                        </td>
                        <td><div className='flex items-center gap-1'>
                                <div>Tên</div>
                                <CgArrowsExchangeAltV size={24} className='text-neutral-1-200 rounded-md hover:text-neutral-1-300 hover:bg-neutral-3-300 cursor-pointer'
                                    onClick={() => sorting("firstName", staffs, setSortData, order, setOrder)}/> 
                        </div></td>
                        <td><div className='flex items-center gap-1'>
                                <div>Họ</div>
                                <CgArrowsExchangeAltV size={24} className='text-neutral-1-200 rounded-md hover:text-neutral-1-300 hover:bg-neutral-3-300 cursor-pointer'
                                    onClick={() => sorting("lastName", staffs, setSortData, order, setOrder)}/> 
                        </div></td>
                        <td className="hidden md:table-cell">
                            <div className='flex items-center gap-1'>
                                    <div>Email</div>
                                    <CgArrowsExchangeAltV size={24} className='text-neutral-1-200 rounded-md hover:text-neutral-1-300 hover:bg-neutral-3-300 cursor-pointer'
                                        onClick={() => sorting("email", staffs, setSortData, order, setOrder)}/> 
                            </div>
                        </td>
                        <td className="hidden md:table-cell">
                            <div className='flex items-center gap-1'>
                        ...
                        <td><div className='flex items-center gap-1'>
                                <div>Tình trạng</div>
                                <CgArrowsExchangeAltV size={24} className='text-neutral-1-200 rounded-md hover:text-neutral-1-300 hover:bg-neutral-3-300 cursor-pointer'
                                    onClick={() => sorting("isActive", staffs, setSortData, order, setOrder)}/> 
                        </div></td>
                        <td>Chi tiết</td>
                    </tr>
                </thead>
                    ...
                    {!loading && currentStaff.map((staff, idx) => {
                        return ( 
                            <tr key={idx} className='h-12 border-b-2 border-neutral-2-200'>
                                ...
                                <td className='hidden md:table-cell md:pl-8 xl:pl-12'>{staff.tour_num} Tour</td>
                                {staff.isActive? 
                                ...
                                <td><FaArrowUpRightFromSquare onClick={() => (navigate(`/manager/staff-detail/${staff.staff_ID}`))} className='ml-5 text-[12px] text-neutral-1-600 hover:text-neutral-1-500 cursor-pointer xl:text-[14px]'/></td>
                            </tr>
                        )
                    })}
                </tbody>
